...
def predict_project_risk(input_dict):
    """
    Takes user input dictionary, processes features, and returns CatBoost risk prediction.
    Returns dict:
    ...
    for col in cat_cols:
        ...
        # Use mapped integer if available, else raw string
        if val in mapping:
            processed_row[col] = val
        elif mapping:
            processed_row[col] = list(mapping.keys())[0]
        else:
            processed_row[col] = val
